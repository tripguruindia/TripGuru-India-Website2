    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-20">
        ...
        <div className="space-y-8">
          ...
          <p className="text-text-secondary text-[15px] font-light leading-relaxed max-w-xs">
        ...
        {/* Quick Links */}
        <div className="space-y-8">
          <h5 className="text-text-primary font-display text-[19px] tracking-widest uppercase">Quick Links</h5>
        ...
        {/* Top Destinations */}
        <div className="space-y-8">
          <h5 className="text-text-primary font-display text-[19px] tracking-widest uppercase">Destinations</h5>
          <ul className="space-y-4">
            {topDestinations.map((dest) => (
              <li key={dest.id}>
                <Link to={`/destinations/${dest.slug}`} className="text-text-secondary hover:text-gold text-[13px] uppercase tracking-widest transition-colors inline-block">
                  {dest.name}
                </Link>
              </li>
            ))}
        ...
        {/* Contact Info */}
        <div className="space-y-8">
          <h5 className="text-text-primary font-display text-[19px] tracking-widest uppercase">Contact</h5>
            ...
            <div className="flex items-center gap-4 group">
              ...
              <div className="flex flex-col">
                <a 
                  ...
                  className="text-text-secondary hover:text-gold text-[15px] font-light transition-colors flex items-center gap-2"
                >
                  {copyStatus === 'copied' ? 'Email Copied' : CONTACT_INFO.email}
                </a>
                <span className="text-[11px] text-text-muted/40 uppercase tracking-widest mt-1">Click to mail or copy</span>
              </div>
            </div>
            <div className="flex items-center gap-4 group">
              <Phone size={18} className="text-gold shrink-0" />
              <a href={`tel:${CONTACT_INFO.phone.replace(/\s/g, '')}`} className="text-text-secondary hover:text-gold text-[15px] font-light transition-colors">
                {CONTACT_INFO.phone}
              </a>
            </div>
          </div>
      ...
      <div className="pt-12 pb-8 border-t border-gold-border/20">
        <h5 className="text-text-primary font-display text-[12px] tracking-[0.25em] uppercase mb-4 text-center md:text-left">Programmatic Regional Desks</h5>
        <div className="flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-3">
          {cityLandingPages.map((page) => (
            <Link
              key={page.slug}
              to={page.path}
              className="text-text-secondary hover:text-gold text-[12px] uppercase tracking-widest transition-colors font-light"
            >
              {page.city}
            </Link>
          ))}
      ...
      <div className="pt-8 flex flex-col md:flex-row justify-between items-center gap-8">
        <div className="text-text-secondary text-[12px] tracking-[0.2em] font-light uppercase text-center md:text-left">
          © 2026 TripGuru · ALL RIGHTS RESERVED
        </div>
        <div className="text-text-secondary text-[12px] tracking-[0.2em] font-light uppercase text-center md:text-right">
